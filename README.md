...
GitOps repo for the sbx-eks-01 EKS Auto Mode cluster. ArgoCD watches `argocd-apps/applicationset.yaml` and auto-discovers per-app folders under `manifests/dev/`.

GitHub-side equivalent of `gitlab-terraform-gcp/sbx-gitops/sbx-02-manifests/`.
...
└── manifests/dev/
    ...
    │   │   ├── ingress.yaml         ← path /shop/, ALB internal, group sbx-shared
    │   │   └── kustomization.yaml
    │   └── overlays/dev/
    │       └── kustomization.yaml   ← images: newName=ECR/sbx-images, newTag=online-shop-<sha>
...
- **Single ECR repo** (HARD RULE `feedback_single_ecr_repo`): `sbx-images` houses everything; per-app discrimination via tag prefix `<app>-<sha>`.
- **Distroless UID 65532**: explicit `runAsUser: 65532` on every container (HARD RULE).
- **Shared ALB** via `alb.ingress.kubernetes.io/group.name: sbx-shared` — both apps land on ONE internal ALB with path-based routing.
...
2. Create `manifests/dev/<app>/overlays/dev/kustomization.yaml` with `images:` newName=ECR/sbx-images, newTag=`<app>-bootstrap`
...
6. Push code in `sbx-<app>` repo → CI pushes image to ECR as `sbx-images:<app>-<sha>`
7. **Manual bump** of overlay newTag to the new SHA (Image Updater not yet wired with ECR auth — see `sbx-cluster-iac/argocd-image-updater.tf` header)
...
kustomize edit set image <app>=784916389752.dkr.ecr.us-east-1.amazonaws.com/sbx-images:<app>-<SHA>
